...
public sealed class C12ReportServiceTests
{
    ...
    [Fact] public async Task CreateAsync_PreservesNewSectionAndResolvedMedicalRecordForVisitQuery()
    { var repository=new FakeRepository(1);var service=new C12ReportService(repository,new C12LegacyAmountConverter(),new AllowConfiguredC12PatientAccessAuthorizer(),new FakeAudit());await service.CreateAsync(Request() with { NewSectionCode="11910" },"user");Assert.Equal("11910",repository.VisitRequest?.NewSectionCode);Assert.Equal("MR1",repository.VisitMedicalRecordNo); }
    private static C12ReportRequest Request()=>new("1150901","1150914",C12Source.OutpatientAndEmergency,0,"A123456789",null);
    ...
    private sealed class FakeRepository(int count):IC12ReportRepository
    {
        ...
        public C12ReportRequest? VisitRequest{get;private set;}
        public string? VisitMedicalRecordNo{get;private set;}
        public Task<IReadOnlyList<C12SectionOption>> QuerySectionOptionsAsync(CancellationToken ct)=>Task.FromResult<IReadOnlyList<C12SectionOption>>([]);
        public Task<string?> ResolveMedicalRecordNoAsync(string value,CancellationToken ct)=>Task.FromResult<string?>("MR1");
        public Task<IReadOnlyList<C12VisitRow>> QueryVisitsAsync(C12ReportRequest r,string m,CancellationToken ct){VisitCalls++;VisitRequest=r;VisitMedicalRecordNo=m;return Task.FromResult<IReadOnlyList<C12VisitRow>>(count<0?[]:[new(new("1150901","1","0101",40000m),"01","醫師","MR1",false)]);}
